...
Next, the program acquires stock specific data from a database with all the cached historical stock prices and load new data from Yahoo Finance API if the data is not in the database. The reason for caching data in the database is to reduce the number of API calls, hence improve the programs' scalability.
